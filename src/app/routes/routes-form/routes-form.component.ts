...
export class RoutesFormComponent
  ...
  constructor(
    ...
    super(routesService, message, activatedRoute);
  }
  ...
  save(): void {
    const selectedPoints = this.points.filter(
      (item) => item.direction === 'right'
    );
  ...
  loadPointsFromCustomer(
    ...
    this.stopsService
      ...
      .subscribe(
        (stops) => {
          this.points = this.mapStopsToTransferItems(stops.results).concat(
            this.selectedPoints
          );
