...
export class SelectableBranchOfficeService {
  customers: Customer[] = [];
  ...
  loadMoreCustomers(): void {
    ...
    this.customersService.searchBranchOffices({
      limit: 50,
  ...
  private setupSearch(): void {
    this.searchCustomerSubject.pipe(debounceTime(2000)).subscribe((search) => {
      this.customersService.searchBranchOffices({limit: 50}, search).subscribe((result) => {
